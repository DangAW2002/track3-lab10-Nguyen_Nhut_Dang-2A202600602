    ...
    for breaker in gateway.breakers.values():
        ...
        for entry in breaker.transition_log:
            if entry["to"] == "open":
                ...
                # Type system note: ts is float
                recovery_times.append((float(entry["ts"]) - float(open_ts)) * 1000.0)
                open_ts = None
    ...
    for breaker in gateway.breakers.values():
        for entry in breaker.transition_log:
            if entry.get("to") == "open":
                total_opens += 1
    metrics.circuit_open_count = total_opens
    
    # Calculate recovery_time_ms
...
def run_simulation(config: LabConfig, queries: list[str]) -> RunMetrics:
    ...
    for scenario in config.scenarios:
        ...
        if scenario.name == "primary_timeout_100":
            ...
            passed = result.availability > 0.7
        elif scenario.name == "all_healthy":
            passed = result.availability > 0.95 and result.error_rate < 0.05
        else:
